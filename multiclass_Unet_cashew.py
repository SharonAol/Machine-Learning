...
print("Class values in the dataset are ... ", np.unique(
    y_train))  # 0 is the background/few unlabeled
...
# y_test_cat = np.float32(y_test_cat)
...
#         # y_true, y_pred shape is (IMG_CHANNELS,IMG_WIDTH,IMG_HEIGHT,n_classes)
...
#         return tf.keras.metrics.categorical_crossentropy(y_true, y_pred) * tf.keras.metrics.Sum(y_true * Kweights, axis=-1)
#     return wcce
...
model = get_model()
model.compile(optimizer='adam', loss=SparseCategoricalFocalLoss(
    gamma=2, from_logits=True), metrics=['accuracy'])
model.summary()
...
# y_train_cat = np.float32(y_train_cat)
# y_test_cat = np.float32(y_test_cat)
print(X_train.shape)
print(X_test.shape)
...
class1_IoU = values[0, 0]/(values[0, 0] + values[0, 1] + values[0, 2] +
                           values[0, 3] + values[1, 0] + values[2, 0] + values[3, 0])
class2_IoU = values[1, 1]/(values[1, 1] + values[1, 0] + values[1, 2] +
                           values[1, 3] + values[0, 1] + values[2, 1] + values[3, 1])
class3_IoU = values[2, 2]/(values[2, 2] + values[2, 0] + values[2, 1] +
                           values[2, 3] + values[0, 2] + values[1, 2] + values[3, 2])
class4_IoU = values[3, 3]/(values[3, 3] + values[3, 0] + values[3, 1] +
                           values[3, 2] + values[0, 3] + values[1, 3] + values[2, 3])
